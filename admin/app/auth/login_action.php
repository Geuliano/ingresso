...
// IPs de proxies reversos confiáveis (ex.: 127.0.0.1 para proxy local).
// Adicione aqui o IP real do seu load balancer/proxy em produção.
...
if (in_array($remoteAddr, $trustedProxies, true) && !empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
    // Só confia no X-Forwarded-For se a requisição vier de um proxy confiável
    $forwarded = trim(explode(',', $_SERVER['HTTP_X_FORWARDED_FOR'])[0]);
    $ip = filter_var($forwarded, FILTER_VALIDATE_IP) ? $forwarded : $remoteAddr;
...
try {
    ...
    if (autenticar($email, $senha)) {
        ...
        resposta(true, 'Login realizado com sucesso.', ['redirect' => BASE_URL]);
    }
