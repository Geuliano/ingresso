...
<script>
    ...
    document.getElementById('formPersonalizacao')?.addEventListener('submit', async (event) => {
        ...
        try {
            const resp = await fetch(controllerUrl, { method: 'POST', body: formData, credentials: 'same-origin' });
            const data = await resp.json();
            if (data.success) {
                adminToast(data.message || 'Configurações atualizadas.', 'success');
                setTimeout(() => window.location.reload(), 1200);
            } else {
                adminAlert(data.message || 'Falha ao salvar.', 'danger');
            }
        } catch (error) {
            adminAlert(error.message || 'Erro inesperado ao salvar.', 'danger');
        } finally {
            btn.disabled = false;
