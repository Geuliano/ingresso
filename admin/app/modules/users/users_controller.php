...
if ($action === 'create') {
    ...
    try {
        ...
        if (!empty($_FILES['avatar']['name'] ?? '')) {
            if ($erroAvatar = validarAvatarUpload($_FILES['avatar'])) {
                throw new RuntimeException($erroAvatar);
            }
        ...
        $msg = $e->getMessage() ?: 'Erro ao criar usuário.';
...
if ($action === 'update') {
    ...
    if (!empty($_FILES['avatar']['name'] ?? '')) {
        if ($erroAvatar = validarAvatarUpload($_FILES['avatar'])) {
            jsonResponse(false, $erroAvatar);
        }
        try {
            $upload = processAvatarUpload($_FILES['avatar'], $id, $avatarDir);
        } catch (Throwable $e) {
            jsonResponse(false, $e->getMessage() ?: 'Falha no upload do avatar.');
        }
...
function processAvatarUpload(array $file, int $idUser, string $avatarDir)
{
    ...
    if ($err !== UPLOAD_ERR_OK) {
        throw new RuntimeException('Erro no upload (código ' . $err . ').');
    }

    $ext = strtolower(pathinfo($file['name'] ?? '', PATHINFO_EXTENSION));
    $permitidas = ['jpg', 'jpeg', 'png', 'webp'];
    if (!in_array($ext, $permitidas, true)) {
        throw new RuntimeException('Formato de avatar inválido. Use JPG, PNG ou WEBP.');
    }

    if (!is_dir($avatarDir) && !@mkdir($avatarDir, 0775, true)) {
        throw new RuntimeException('Não foi possível criar a pasta de avatars.');
    }

    if (!is_uploaded_file($file['tmp_name'] ?? '')) {
        throw new RuntimeException('Arquivo temporário de upload é inválido.');
    }

    // Valida que o conteúdo é realmente uma imagem (magic bytes via GD)
    $info = @getimagesize($file['tmp_name']);
    $mimePermitidos = ['image/jpeg', 'image/png', 'image/webp'];
    if (!$info || !in_array($info['mime'], $mimePermitidos, true)) {
    ...
    if (!is_writable($destDir)) {
        throw new RuntimeException("Pasta de upload não é gravável: {$destDir}");
    }

    $tmp = $file['tmp_name'] ?? '';

    if (!@move_uploaded_file($tmp, $destino)) {
        // Fallbacks para ambientes com move_uploaded_file restrito
        if (!@rename($tmp, $destino)) {
            if (!@copy($tmp, $destino)) {
                throw new RuntimeException("Falha ao mover o avatar para o destino final. Destino: {$destino} | Tmp: {$tmp}");
            }
            @unlink($tmp);
        }
    }
...
/**
 * Retorna mensagem de erro de upload de avatar ou null se estiver ok.
 */
function validarAvatarUpload(array $file): ?string
{
    $err = $file['error'] ?? UPLOAD_ERR_NO_FILE;
    if ($err !== UPLOAD_ERR_OK) {
        $map = [
            UPLOAD_ERR_INI_SIZE => 'Arquivo maior que o permitido pelo servidor.',
            UPLOAD_ERR_FORM_SIZE => 'Arquivo excede o limite configurado.',
            UPLOAD_ERR_PARTIAL => 'Upload interrompido. Tente novamente.',
            UPLOAD_ERR_NO_FILE => 'Nenhum avatar selecionado.',
            UPLOAD_ERR_NO_TMP_DIR => 'Pasta temporária ausente no servidor.',
            UPLOAD_ERR_CANT_WRITE => 'Falha ao gravar o arquivo no disco.',
            UPLOAD_ERR_EXTENSION => 'Upload bloqueado por extensão.',
        ];
        return $map[$err] ?? 'Falha desconhecida ao enviar o avatar.';
    }

    $ext = strtolower(pathinfo($file['name'] ?? '', PATHINFO_EXTENSION));
    $permitidas = ['jpg', 'jpeg', 'png', 'webp'];
    if (!in_array($ext, $permitidas, true)) {
        return 'Formato de avatar não permitido. Use JPG, PNG ou WEBP.';
    }

    $size = $file['size'] ?? 0;
    if ($size > 5 * 1024 * 1024) {
        return 'Avatar maior que 5MB. Escolha um arquivo menor.';
    }
