...
$defaultAvatar = BASE_URL . 'public/assets/images/users/default.svg';
    ...
    <div class="card-header">
        <div class="row align-items-center g-2">
            <div class="col">
                ...
                <p class="text-muted mb-0">Gerencie contas de acesso, níveis e dados cadastrais dos usuários do painel.</p>
            </div>
            <div class="col-auto">
                <button type="button" class="btn bg-primary text-white"
                    onclick="openUserModal('create')" <?= $possuiNiveis ? '' : 'disabled' ?>>
                    <i class="las la-plus me-1"></i> Novo Usuário
                </button>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <button type="button"
                                    ...
                                    onclick="openUserModal('edit', <?= (int)$u['id'] ?>, this.dataset.avatar)">
...
<script>
    ...
    const avatarUploadsBase = '<?= htmlspecialchars($avatarUrlBase) ?>';
    const defaultAvatarUrl = '<?= htmlspecialchars($defaultAvatar) ?>';
    ...
    function resetUsuarioForm(keepPreview = false) {
        ...
        if (!keepPreview) {
            ...
            if (preview) {
                ...
                preview.dataset.currentAvatar = defaultAvatarUrl;
            }
        }
        const campoAvatar = document.getElementById('campo_avatar');
        if (campoAvatar) {
            campoAvatar.value = '';
        }
    ...
    function resolveAvatarUrl(url) {
        ...
        if (idx !== -1) {
            ...
            return avatarUploadsBase + filename;
        }
    ...
    function attachAvatarPreview() {
        const input = document.getElementById('campo_avatar');
        const preview = document.getElementById('previewAvatar');
        ...
        input.addEventListener('change', () => {
            ...
            preview.src = preview.dataset.currentAvatar || defaultAvatarUrl;
        });
    }

    async function openUserModal(mode, userId = null, avatarUrl = '') {
        const modal = bootstrap.Modal.getOrCreateInstance('#modalUsuario');
        resetUsuarioForm(mode === 'edit');
        const preview = document.getElementById('previewAvatar');
        if (preview) {
            const resolved = avatarUrl ? resolveAvatarUrl(avatarUrl) : defaultAvatarUrl;
            preview.src = resolved;
            preview.dataset.currentAvatar = resolved;
            preview.onerror = () => { preview.onerror = null; preview.src = defaultAvatarUrl; };
        }
        const form = document.getElementById('formUsuario');
        ...
        if (mode === 'edit') {
            ...
            try {
                ...
                if (!data.success) {
                    adminAlert(data.message || 'Não foi possível carregar o usuário.', 'danger');
                    return;
                }
                fillUsuarioForm(data.usuario || {}, avatarUrl);
                modal.show();
            } catch (err) {
                adminAlert(err.message || 'Erro ao buscar usuário.', 'danger');
            }
        } else {
            if (!possuiNiveis) {
                adminAlert('Cadastre um nível de acesso antes de criar usuários.', 'warning');
                return;
            }
            modal.show();
        }
    ...
    function fillUsuarioForm(usuario, fallbackAvatar = '') {
        ...
        const preview = document.getElementById('previewAvatar');
        if (preview) {
            const resolved = resolveAvatarUrl(usuario.avatar_url || fallbackAvatar || defaultAvatarUrl);
            preview.src = resolved;
            preview.dataset.currentAvatar = resolved;
            preview.onerror = () => { preview.onerror = null; preview.src = defaultAvatarUrl; };
        }
        ...
        if (possuiNiveis) {
            ...
            if (!optionExists && usuario.nivel) {
                ...
                opt.textContent = 'N?vel atual (inativo)';
                opt.selected = true;
    ...
    async function submitUsuarioForm(event) {
        ...
        if (!possuiNiveis) {
            adminAlert('Cadastre um nível de acesso antes de salvar.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                adminToast(data.message || 'Usuário salvo.', 'success');
    ...
    async function confirmDelete(id) {
        if (!id) {
            adminAlert('Usuário inválido.', 'danger');
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                ...
                adminAlert(res.message || 'Erro ao excluir usuário.', 'danger');
            }
        } catch (err) {
            adminAlert(err.message || 'Erro ao conectar com o servidor.', 'danger');
        }
    }
</script>
