...
<div class="card">
    <div class="card-header">
        <div class="row align-items-center g-2">
            <div class="col">
                <h4 class="card-title mb-0">Módulos e permissões</h4>
                <p class="text-muted mb-0">Organize módulos, configure hierarquia e gerencie permissões por nível.</p>
            </div>
            <div class="col-auto">
                <button type="button" class="btn bg-primary text-white" data-bs-toggle="modal"
                    ...
                    <i class="las la-plus me-1"></i> Novo Módulo
                </button>
            </div>
        </div>
    </div>
...
let reorderInProgress = false;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        ['datatable.page', 'datatable.sort', 'datatable.search'].forEach(function (evt) {
            tabelaModulos.on(evt, function () { if (window.initTooltips) window.initTooltips(); });
        });
    ...
    function resetModuloForm() {
        ...
        if (campoVerMenu) campoVerMenu.value = '1';
    }
    ...
    async function openEditModule(id) {
        ...
        try {
            ...
            if (!data.success) {
                adminAlert(data.message || 'Não foi possível carregar o módulo.', 'danger');
                return;
            }
            ...
            adminAlert(err.message || 'Erro inesperado ao carregar o módulo.', 'danger');
        }
    }
    ...
    async function submitModuloForm(e) {
        ...
        try {
            ...
            const data = await parseJsonResponse(resp);
            if (data.success) {
                adminToast(data.message || 'Módulo salvo.', 'success');
                setTimeout(() => window.location.reload(), 700);
            } else {
                adminAlert(data.message || 'Falha ao salvar módulo.', 'danger');
            }
        } catch (err) {
    ...
    async function reorderModule(id, direction) {
        ...
        try {
            ...
            const data = await parseJsonResponse(resp);
            if (!data.success) {
                adminAlert(data.message || 'Não foi possível reordenar o módulo.', 'danger');
                return;
            }
            adminToast(data.message || 'Ordem atualizada.', 'success');
            setTimeout(() => window.location.reload(), 600);
        } catch (err) {
            adminAlert(err.message || 'Erro inesperado ao reordenar o módulo.', 'danger');
        } finally {
            reorderInProgress = false;
        }
    }

    async function confirmDeleteModule(id) {
        if (!id) { adminAlert('Módulo inválido.', 'danger'); return; }
        ...
        try {
            const resp = await fetch(controllerUrl, { method: 'POST', body: fd, credentials: 'same-origin' });
            const data = await parseJsonResponse(resp);
            if (!data.success) { adminAlert(data.message || 'Não foi possível excluir o módulo.', 'danger'); return; }
            adminToast(data.message || 'Módulo excluído.', 'success');
            setTimeout(() => window.location.reload(), 800);
        } catch (err) {
            adminAlert(err.message || 'Erro inesperado ao excluir módulo.', 'danger');
        }
    }
</script>
